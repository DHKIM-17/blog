...
try {
  const envLocal = fs.readFileSync('.env.development.local', 'utf8');
  envLocal.split('\n').forEach(line => {
    const match = line.match(/^([^=]+)="?(.*?)"?$/);
    if (match && !process.env[match[1]]) process.env[match[1]] = match[2];
  });
...
// 2. NOW import postgres and R2 client
...
const urls = [
  "https://m.blog.naver.com/cine_ma/223551314454"
];
...
async function processUrl(url, index) {
  ...
  console.log(`   ✅ DB Inserted live to Vercel Postgres!`);
}
...
async function runAll() {
    ...
    console.log(`\n🎉 The article has been successfully migrated to 경기장 투어!`);
    
    // Wait slightly to let connections flush
